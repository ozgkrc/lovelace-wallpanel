...
const logger = {
  ...
  downloadMessages: function () {
    const data = new Blob([stringify(logger.messages)], {
      type: "text/plain"
    });
  ...
  error: function () {
    ...
    if (config.alert_errors) {
      alert("Wallpanel error: ".concat(stringify(arguments)));
    }
  }
};
...
function updateConfig() {
  ...
  if (config.profiles && browserId && config.profiles["device.".concat(browserId)]) {
    const profile = "device.".concat(browserId);
    config = mergeConfig(config, config.profiles[profile]);
    logger.debug("Profile set from device: ".concat(profile));
  }
  if (config.profiles) {
    ...
    for (let i = 0; i < userIds.length; i++) {
      ...
      if (user) {
        ...
        if (config.profiles["user.".concat(user)]) {
          const profile = "user.".concat(user);
          config = mergeConfig(config, config.profiles[profile]);
          logger.debug("Profile set from user: ".concat(profile));
          break;
        }
      }
    }
  ...
  if (config.profiles && profile_entity && (elHass.hass || elHass.__hass).states[profile_entity] && config.profiles[(elHass.hass || elHass.__hass).states[profile_entity].state]) {
    ...
    config = mergeConfig(config, config.profiles[profile]);
    logger.debug("Profile set from entity state: ".concat(profile));
  }
...
function navigate(path) {
  let keepSearch = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
...
function initWallpanel() {
  ...
  class WallpanelView extends HuiView {
    ...
    updateShadowStyle() {
      ...
      const maxX = this.infoContainer.offsetWidth - parseInt(computed.paddingLeft) - parseInt(computed.paddingRight) - this.infoBox.offsetWidth;
      const maxY = this.infoContainer.offsetHeight - parseInt(computed.paddingTop) - parseInt(computed.paddingBottom) - this.infoBox.offsetHeight;
      let host = "";
      if (config.style) {
      ...
      let classCss = "";
      for (const className in classStyles) {
        classCss += ".".concat(className, " {\n");
        for (const attr in classStyles[className]) {
          classCss += "".concat(attr, ": ").concat(classStyles[className][attr], ";\n");
        }
        classCss += "}\n";
      }
    ...
    moveAroundCorners() {
      let correctPostion = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
      ...
      const computed = getComputedStyle(this.infoContainer);
      const x = [2, 3].includes(this.lastCorner) ? this.infoContainer.offsetWidth - parseInt(computed.paddingLeft) - parseInt(computed.paddingRight) - this.infoBox.offsetWidth : 0;
      const y = [1, 2].includes(this.lastCorner) ? this.infoContainer.offsetHeight - parseInt(computed.paddingTop) - parseInt(computed.paddingBottom) - this.infoBox.offsetHeight : 0;
      this.moveInfoBox(x, y, fadeDuration);
    }
    moveInfoBox(x, y) {
      let fadeDuration = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
    ...
    restartKenBurnsEffect() {
      ...
      const duration = Math.ceil(config.image_animation_ken_burns_duration || (config.display_time + config.crossfade_time * 2) * 1.2);
      const animation = config.image_animation_ken_burns_animations[Math.floor(Math.random() * config.image_animation_ken_burns_animations.length)];
    ...
    handleMediaError(element, error) {
      ...
      logger.error("Failed to load media: ".concat(element.mediaUrl, ":"), error);
      this.displayMessage("Failed to load media: ".concat(element.mediaUrl, ": ").concat(error), 5000);
    }
    ...
    setMediaDataInfo() {
      let mediaElement = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
      if (!mediaElement) {
    ...
    async updateMediaList() {
      ...
      try {
        ...
        logger.debug("Image list from ".concat(sourceType, " is now:"), wp.mediaList);
        if (callback) {
          callback();
        }
      } catch (error) {
        ...
        logger.warn("Failed to update image list from ".concat(sourceType, ":"), error);
        if (retryCount < maxRetries) {
          logger.warn("Retrying image list update in ".concat(retryDelay / 1000, " seconds (attempt ").concat(retryCount + 1, "/").concat(maxRetries, ")..."));
          setTimeout(() => wp.updateMediaList(callback, true, retryCount + 1), retryDelay);
        } else {
          const errorMsg = "Failed to update image list from ".concat(config.image_url, " after ").concat(maxRetries, " retries: ").concat(error.message || stringify(error));
    ...
    async findMedias(mediaContentId) {
      ...
      try {
        ...
        logger.warn("Error browsing media ".concat(mediaContentId, ":"), error);
    ...
    async updateMediaListFromMediaSource() {
      ...
      try {
        ...
        throw new Error("Failed to update image list from ".concat(config.image_url, ": ").concat(error.message || stringify(error)));
      }
    }
    async updateMediaListFromUnsplashAPI() {
      const urls = [];
      const requestUrl = "".concat(config.image_url, "&count=30");
      ...
      try {
        ...
        json.forEach(entry => {
          logger.debug(entry);
          const url = "".concat(entry.urls.raw, "&w=${width}&h=${height}&auto=format");
          urls.push(url);
    ...
    async updateMediaListFromImmichAPI() {
      ...
      const wp = this;
      const screenOrientation = this.screensaverContainer.clientWidth >= this.screensaverContainer.clientHeight ? "landscape" : "portrait";
      ...
      function processAssets(assets) {
        let folderName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
        assets.forEach(asset => {
          ...
          const url = "".concat(apiUrl, "/assets/").concat(asset.id, "/").concat(resolution);
      ...
      try {
        if (config.immich_persons && config.immich_persons.length) {
          const orPersonNames = config.immich_persons.map(entry => (Array.isArray(entry) ? entry : [entry]).map(v => v.toLowerCase()));
          ...
          while (hasNextPage) {
            const peopleData = await wp._immichFetch("".concat(apiUrl, "/people?size=1000&page=").concat(page));
          ...
          for (const personNames of orPersonNames) {
            const personIds = personNames.map(name => personNameToId[name]).filter(id => {
              if (!id) logger.error("Person not found in immich: ".concat(name));
              return !!id;
            });
            if (personIds.length > 0) {
              logger.debug("Searching asset metadata for persons: ", personIds);
              const searchResults = await wp._immichFetch("".concat(apiUrl, "/search/metadata"), {
                method: "POST",
                body: JSON.stringify({
                  personIds: personIds,
                  withExif: true,
              ...
              if (!searchResults.assets.count) {
                logger.warn("No media items found in immich that contain all these people: ".concat(personNames));
              } else {
          ...
          const allMemories = await wp._immichFetch("".concat(apiUrl, "/memories?type=on_this_day"));
          ...
          const allTags = await wp._immichFetch("".concat(apiUrl, "/tags"));
          logger.debug("Got immich API response", allTags);
          const tagIds = allTags.filter(tag => {
          ...
          const allAlbums = await wp._immichFetch("".concat(apiUrl, "/albums?shared=").concat(config.immich_shared_albums));
          logger.debug("Got immich API response", allAlbums);
          const albumIdsToFetch = allAlbums.filter(album => {
    ...
    async updateMediaFromUrl(element, url) {
      var _this = this;
      ...
      let useFetch = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
      ...
      const loadMediaWithElement = async elem => {
        ...
        if (!loadEventName) {
          logger.error("Unsupported element tag \"".concat(elem.tagName, "\""));
          return;
        }
        ...
        if (useFetch) {
          ...
          if (!response.ok) {
            logger.error("Failed to load ".concat(elem.tagName, " \"").concat(url, "\""), response);
            return;
          }
      ...
      const createFallbackElement = function (currentElem) {
        let tagName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
        if (!tagName) {
        ...
        Object.entries(currentElem).filter(_ref => {
          let [key] = _ref;
          return !(key in HTMLElement.prototype);
        }).forEach(_ref2 => {
          let [key, value] = _ref2;
          return fallbackElem[key] = value;
        });
        [...currentElem.attributes].filter(attr => attr.name !== "src").forEach(attr => fallbackElem.setAttribute(attr.name, attr.value));
        if (tagName.toLowerCase() === "video") {
          // Do not set muted to true or the following error can occur:
          // Uncaught (in promise) DOMException: The play() request was interrupted because video-only background media was paused to save power. https://goo.gl/LdLk22
          Object.assign(fallbackElem, {
            preload: "auto",
            muted: false,
      ...
      const handleFallback = async function (currentElem) {
        let mediaType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
        let originalError = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
        ...
        try {
          ...
          _this.handleMediaError(currentElem, originalError || e);
        }
      };
    ...
    async updateMediaFromMediaSource(element) {
      try {
        ...
        const matchedType = (_result$mime_type = result.mime_type) === null || _result$mime_type === void 0 ? void 0 : _result$mime_type.match(/^(image|video)\//);
        const mediaType = {
          image: "img",
          video: "video"
        ...
        logger.error("media_source/resolve_media error for ".concat(element.mediaUrl, ":"), error);
      }
    }
    ...
    startPlayingActiveMedia() {
      ...
      const cleanupListeners = () => {
        if (playbackListeners) {
          Object.entries(playbackListeners).forEach(_ref3 => {
      ...
      if (!config.video_loop) {
        ...
        Object.entries(playbackListeners).forEach(_ref4 => {
      ...
      activeElem.play().catch(e => {
        ...
        if (activeElem === this.getActiveMediaElement()) {
          logger.error("Failed to play media \"".concat(activeElem.src, "\":"), e);
        }
      });
    }
    ...
    _switchActiveMedia(newElement) {
      let crossfadeMillis = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
      this.lastMediaUpdate = Date.now();
    ...
    displayMessage(message) {
      let timeout = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 15000;
      this.hideMessage();
    ...
    stopScreensaver() {
      let fadeOutTime = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0.0;
    ...
    updateScreensaver() {
      ...
      if (config.debug) {
        ...
        if (activeElement) {
          ...
          if (mediaInfo) {
            html += "<b>Media info:</b> ".concat(JSON.stringify(mediaInfo), "<br/>");
          }
        }
...
function waitForEnv(callback) {
  let startTime = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
  const now = Date.now();
  if (startTime === null) {
    startTime = now;
  }
